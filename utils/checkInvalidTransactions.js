...
export const transactions = async () => {
  usdc.on("Transfer", async (from, to, value, event) => {
    ...
    const existingTx = await TransactionHistory.findOne({
      where: {
        transHash: event.log.transactionHash,
      },
    });
    ...
    console.log("Id: ", findUser?.id, "Private Key: ", findUser?.privateKey);
    ...
    if (
      (findBankKYC || findNonBankKYC) &&
      to.toLowerCase() !== process.env.DISPUTE_WALLET_ADDRESS.toLowerCase()
      && from.toLowerCase() !== process.env.DISPUTE_WALLET_ADDRESS.toLowerCase()
    ) {
    ...
    if (
      ...
      await TransactionHistory.create({
        ...
        toWalletAddress: to,
        status: "INVALID",
        transHash: event.log.transactionHash || null,
        initiator: null,
        isAuthorized: 0,
        toDisputeWallet: 1,
        isDispute: 1,
